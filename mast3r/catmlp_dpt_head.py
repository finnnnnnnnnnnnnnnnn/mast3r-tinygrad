...
from mast3r.utils.pixel_shuffle import pixel_shuffle
...
def reg_desc(desc, mode):
    if 'norm' in mode:
        desc = desc / desc.pow(2).sum(axis=-1, keepdim=True).sqrt() # -> Frobenius norm
    else:
...
class Cat_MLP_LocalFeatures_DPT_Pts3d(PixelwiseTaskWithDPT):
    ...
    def __call__(self, decout, img_shape):
        ...
        cat_output = tinygrad.Tensor.cat(enc_output, dec_output, dim=-1)  # concatenate
        H, W = img_shape
        ...
        local_features = pixel_shuffle(local_features, self.patch_size)  # B,d,H,W

        # post process 3D pts, descriptors and confidences
        out = tinygrad.Tensor.cat(pts3d, local_features, dim=1)
        if self.postprocess:
