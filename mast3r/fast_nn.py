...
def bruteforce_reciprocal_nns(A, B, device='cuda', block_size=None, dist='l2'):
    ...
    if dist == 'l2':
        dist_func = tinygrad.cdist
        argmin = tinygrad.Tensor.min
    elif dist == 'dot':
        def dist_func(A, B):
            return A @ B.T

        def argmin(X, dim):
            sim = X.max(axis=dim)  # Max values along dimension 0
            nn = X.argmax(axis=dim)
            return sim.neg(), nn
    else:
    ...
    if block_size is None or len(A) * len(B) <= block_size**2:
        ...
        dis_A = tinygrad.Tensor.full((A.shape[0],), float('inf'), dtype=A.dtype).contiguous()
        dis_B = tinygrad.Tensor.full((B.shape[0],), float('inf'), dtype=B.dtype).contiguous()
        nn_A = tinygrad.Tensor.full((A.shape[0],), -1, dtype=tinygrad.dtypes.int64).contiguous()
        nn_B = tinygrad.Tensor.full((B.shape[0],), -1, dtype=tinygrad.dtypes.int64).contiguous()
        ...
        for i in range(number_of_iteration_A):
            ...
            for j in range(number_of_iteration_B):
                ...
                dis_A_section = dis_A[i * block_size : (i + 1) * block_size]
                dis_B_section = dis_B[j * block_size:(j + 1) * block_size]
                dis_A[i * block_size : (i + 1) * block_size] = dis_A_section.where(col_mask.logical_not(), min_A_i)
                dis_B[j * block_size:(j + 1) * block_size] = dis_B_section.where(line_mask.logical_not(), min_B_j)

                nn_A_section = nn_A[i * block_size:(i + 1) * block_size]
                nn_A[i * block_size:(i + 1) * block_size] = nn_A_section.where(col_mask.logical_not(), argmin_A_i + (j * block_size))
                nn_B_section = nn_B[j * block_size:(j + 1) * block_size]
                nn_B[j * block_size:(j + 1) * block_size] = nn_B_section.where(line_mask.logical_not(), argmin_B_j + (i * block_size))
    nn_A = nn_A.numpy()
    nn_B = nn_B.numpy()
    return nn_A, nn_B
...
def fast_reciprocal_NNs(pts1, pts2, subsample_or_initxy1=8, ret_xy=True, pixel_tol=0, ret_basin=False,
    ...
    if isinstance(subsample_or_initxy1, int) and pixel_tol == 0:
        ...
        if isinstance(x1, tinygrad.Tensor):
            x1 = x1.numpy()
        if isinstance(y1, tinygrad.Tensor):
            y1 = y1.numpy()
        max_iter = 1
    ...
    if 'dist' in matcher_kw or 'block_size' in matcher_kw \
            ...
            or (isinstance(device, tinygrad.device) and device.type.startswith('cuda')):
    ...
    while notyet.any():
        print("niter: ", niter)
        print("egers")
        # _, xy2[notyet] = to_numpy(tree2.query(pts1[tinygrad.Tensor(xy1[notyet])], **matcher_kw))
        
        # Extracting indices where 'notyet' condition holds
        notyet_pts = xy1[notyet]
        print("notyet_pts:", notyet_pts)

        # Converting to tinygrad Tensor
        notyet_tensor = tinygrad.Tensor(notyet_pts)
        print("notyet_tensor:", notyet_tensor)

        # Querying tree2 with the points that match 'notyet' in pts1
        query_result = tree2.query(pts1[notyet_tensor], **matcher_kw)
        print("query_result:", query_result)

        # Converting the query result to numpy
        _, numpy_result = to_numpy(query_result)
        # import sys
        # np.set_printoptions(threshold=sys.maxsize)
        print("numpy_result:", numpy_result)

        # Assigning result back to xy2 at 'notyet' positions
        xy2[notyet] = numpy_result
        print("Updated xy2:", xy2)

        if not ret_basin:
            notyet &= (old_xy2 != xy2)  # remove points that have converged
        _, xy1[notyet] = to_numpy(tree1.query(pts2[tinygrad.Tensor(xy2[notyet])], **matcher_kw))
        if ret_basin:
